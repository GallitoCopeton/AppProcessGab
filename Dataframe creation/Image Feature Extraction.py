...
for i, marker in enumerate(markers):
    ...
    # Thresholded image
    _, imgBina = cv2.threshold(imgGray, 120, 255, cv2.THRESH_BINARY)
...
for i in range(0, 4):
    dfForTraining.loc[dfForTraining[f'distanceQ{i+1}']
                      < 4, f'areaQ{i+1}'] = 0
# %%
dfForTraining.to_csv('../Dataframes/FewerFeatures.csv')
# %%
totalArea = dfForTraining.loc[:, 'areaTotal']
totalAreaMean = totalArea.mean()
totalAreaStd = totalArea.std()
totalArea1Std = totalAreaMean+(1*totalAreaStd)
totalArea2Std = totalAreaMean+(2*totalAreaStd)
totalArea3Std = totalAreaMean+(3*totalAreaStd)
dfForTraining = dfForTraining[dfForTraining.loc[:,
                                                'areaTotal'] < totalArea3Std]
sns.violinplot(y=dfForTraining['distanceTotal'],x=dfForTraining['diagnostic'],data=dfForTraining)
# %%

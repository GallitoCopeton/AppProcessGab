...
def areaAnalysisAreasV2(imgBin, blobs, area, markerNum):
    # ths = [26,30,36,26,35,22]
    # ths = [45,48,47,36,35,30]
    ths = [30, 30, 30, 30, 30, 25]
    if (area < ths[markerNum]) or (len(blobs) < 2):
        return 'N'
    else:
        return 'P'
    
def areaAnalysisAreasV22(imgBin, blobs, area, name):
    # ths = [26,30,36,26,35,22]
    # ths = [45,48,47,36,35,30]
    ths = {
        'ESAT6': 45,
    ...
    if (area < ths[name]) or (len(blobs) < 2):
        return 'N'
    else:
        return 'P'
...
def quadrantBlobAnalisys(images, bgrImages):
    ...
    for i, image in enumerate(images):
        ...
        if len(contours) > 0 and len(contours[0]) > 4:
            ...
            # Data storage
            measurementsDict[f'distanceQ{i+1}'] = distance
            measurementsDict[f'areaQ{i+1}'] = quadrantArea
            measurementsDict[f'perimeterQ{i+1}'] = quadrantPerimeter
            measurementsDict[f'verticalDiagQ{i+1}'] = verticalDiag
            measurementsDict[f'horizontalDiagQ{i+1}'] = horizontalDiag

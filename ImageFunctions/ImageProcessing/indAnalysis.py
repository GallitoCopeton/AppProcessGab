...
def makeNewMask(mask):
    maskCopy = mask.copy()
    h = maskCopy.shape[1]
    kSize = 30
    percentage = int(.16*h)
    maskCopy[percentage:percentage +
                        kSize, percentage:percentage+kSize] = 0
    maskCopy[percentage:percentage +
                        kSize, h-percentage-kSize:h-percentage] = 0
    maskCopy[h-percentage-kSize:h -
                        percentage, percentage:percentage+kSize] = 0
    maskCopy[h-percentage-kSize:h -
                        percentage, h-percentage-kSize:h-percentage] = 0
    return andOperation(mask, maskCopy)
...
def getBackgroundColor(markerGray, kSize):
...
def getBlobInfo(markersNot):
    sites = ['ESAT6', 'CF', 'RV', 'Control']
    results = []
    for i, markerNot in enumerate(markersNot):
        markerArea = 0
        marker_blobs = blobDetect(markerNot)
        for markerBlob in marker_blobs:
            markerArea += markerBlob.size
        results.append(markerArea)
    return pd.DataFrame(results, index=sites, columns=['Pixels'])

def getHighestPixelQuadrants(quadrants):
    highestPixels = []
    for quadrant in quadrants:
        highestPixelsInQuadrant = quadrant.max()
        
        highestPixels.append(highestPixelsInQuadrant)

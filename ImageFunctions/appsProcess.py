...
def markerClusterProcessing(markers):
    ...
    for marker in markers:
        ...
        t = inA.getBackgroundColor(clusterGray, percentage=.35)+15
...
def showBothProcesses(card: np.ndarray, markers: list, markerDirectProcess: list, markerClusterProcess: list, figsize=5, save=False, folderName='./'):
...
def getBlobInfo(clustersNot):
    columns = ['ESAT6', 'CF', 'RV', 'Control']
    results = {
        'ESAT6': 0,
        'CF': 0,
        'RV': 0,
    ...
    df = pd.DataFrame()
    for i, clusterNot in enumerate(clustersNot):
        clusterMarker_area = 0
        clusterMarker_blobs = inA.blobDetect(clusterNot)
        for clusterMarker_blob in clusterMarker_blobs:
            clusterMarker_area += clusterMarker_blob.size
        if i == 0:
            results['ESAT6'] += clusterMarker_area
        elif i == 1:
            results['CF'] += clusterMarker_area
        elif i == 2:
            results['RV'] += clusterMarker_area
        elif i == 3:
            results['Control'] += clusterMarker_area

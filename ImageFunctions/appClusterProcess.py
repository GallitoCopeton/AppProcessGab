...
def markerClusterProcessing(markers):
    ...
    for marker in markers:
        ...
        t = inA.getBackgroundColor(clusterGray, 6)
        print(t)
...
def markerClusterProcessing2(markers, k, attempts):

    criteria = (cv2.TERM_CRITERIA_MAX_ITER, 100000, 100000)
    erodeKernel = cv2.getStructuringElement(cv2.MORPH_CROSS, (2, 2))
    dilateKernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))

    markersNot = []
    for i, marker in enumerate(markers):
        clusterRecon = pP.clusterReconstruction(marker, criteria, k, attempts)
        maskedMarker = inA.andOperation(clusterRecon, mask)
        quadrants = inA.imageQuadrantSplit(maskedMarker)
        highestPixel = np.array(
            [cv2.cvtColor(quadrant, cv2.COLOR_BGR2GRAY).max()+1 for quadrant in quadrants])
        quadrantsNot = []
        for quadrant in quadrants:
            quadrantGray = cv2.cvtColor(quadrant, cv2.COLOR_BGR2GRAY)
            _, quadrantBin = cv2.threshold(
                quadrantGray, highestPixel.min()-1, 255, cv2.THRESH_BINARY)
            quadrantTrans = cv2.morphologyEx(
                quadrantBin, cv2.MORPH_ERODE, erodeKernel)
            quadrantTrans = cv2.morphologyEx(
                quadrantTrans, cv2.MORPH_DILATE, dilateKernel)
            quadrantNot = cv2.bitwise_not(quadrantTrans)
            quadrantsNot.append(quadrantNot)
        marker = inA.mergeQuadrants(quadrantsNot)
        markersNot.append(marker)
    return markersNot
